...
# ------------------
complexTuples = True
respectTradingRules = True # specific patch for trading system models
# ------------------
...
class EventLogGenerator:
	...
	def generateTrace(self, eventLog, petriNet, traceNumber):
		...
		badTransitions = [] # PATCH <-- FIX ME LATER!

		# While we have not reached the final marking, fire randomly selected transitions
		while len(fireableTransitions) > 0:

			# PATCH FOR CONTROLLING A BIT MORE THE ARTIFICIAL GENERATION OF BEHAVIOR <--- FIX ME LATER!
		...
		#		u = random.uniform(0, 1)
			# respect random rules 95% of times.
		...
		#	t = random.choice(fireableTransitions)
			...
			if respectTradingRules == True and (t == "t5" or t == "t6" or t == "t7"):
				...
				# Now, select the transitio to fire acording to their quantities!
				if q == q2:
					t = "t5"
				elif q > q2:
					t = "t6"
				...
				binding = None
				#print("Number of bindings: " + str(len(petriNet.transition(t).modes())))
				#print(t + " " + identifierBestBuyOrder + " " + str(priceBestBuyOrder) + " " + str(qty) + identifierBestSellOrder + " " + str(priceBestSellOrder) + " " + str(qty))
				# Select the binding which has these orders!
			...
			if(binding is None):
				print("Error baby, error. Delete trace:" + str(traceNumber + 1))
				break
