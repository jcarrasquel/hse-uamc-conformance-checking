...
numberOfArtificialResources = 25
# ================================
...
#	SELL_ORDER_IDENTIFIERS = []
	...
	petriNet = PetriNet('C_deviation_resource_corruption')
	...
	"discard buy order", "discard sell order", "new buy order", "new sell order"]
	...
	# ===== INCORRECT BEHAVIOR ======
	# Sometimes, orders that enter to the order have quantity 0

	# input/output of t8 - discard buy order
	petriNet.add_input("p3", "t10", Tuple([Variable("o1"), Variable("ts1"), Variable("pr1"), Variable("q1")]))
	petriNet.add_output("p5", "t10", Tuple([Variable("o1"), Variable("ts1"), Variable("pr1"), Value(0)]))

	# input/output of t9 - discard sell order
	petriNet.add_input("p4", "t11", Tuple([Variable("o2"), Variable("ts2"), Variable("pr2"), Variable("q2")]))
